...
public class TTSAPP {
    ...
    private static DnsOverHttps getDns() {
        ...
        if (dns == null) {
            synchronized (TTSAPP.class) {
                if (dns == null) {
                    OkHttpClient temp = TTSAPP.bootClient.newBuilder().cache(getCache("doh", 1024 * 1024 * 100)).build();
...
//                                }
    ...
    public static okhttp3.Cache getCache(String name, int maxSize) {
        ...
        if (!file.exists()) {
            ...
            if (!mkdirs) {
                Log.e(TTSAPP.class.getSimpleName(), "创建文件夹失败");
            }
        }
