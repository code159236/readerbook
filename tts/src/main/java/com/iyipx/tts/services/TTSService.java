...
import static com.iyipx.tts.TTSAPP.getOkHttpClient;
import static com.iyipx.tts.utils.CommonTool.getTime;
...
import com.iyipx.tts.BuildConfig;
import com.iyipx.tts.utils.ByteArrayMediaDataSource;
...
//                    if (TokenHolder.token != null && APP.getBoolean(Constants.USE_PREVIEW, false)) {
                    if (TTSAPP.getBoolean(Constants.USE_PREVIEW, true)) {
                    ...
                    sendConfig(Objects.requireNonNull(this.webSocket), new TtsConfig.Builder(TTSAPP.getInt(Constants.AUDIO_FORMAT_INDEX, 0)).build());
    ...
    public synchronized void sendText(SynthesisRequest request, SynthesisCallback callback) {
        ...
        //设置发送的音质
        int index = TTSAPP.getInt(Constants.AUDIO_FORMAT_INDEX, 0);
        ...
        if (TTSAPP.getBoolean(Constants.USE_CUSTOM_VOICE, true)) {
            name = TTSAPP.getString(Constants.CUSTOM_VOICE, "zh-CN-YunyeNeural");
        }
        int styleIndex = TTSAPP.getInt(Constants.VOICE_STYLE_INDEX, 0);
        TtsStyle ttsStyle = TtsStyleManger.getInstance().get(styleIndex);
        ttsStyle.setStyleDegree(TTSAPP.getInt(Constants.VOICE_STYLE_DEGREE, 100));
        ttsStyle.setVolume(TTSAPP.getInt(Constants.VOICE_VOLUME, 100));
        boolean useDict = TTSAPP.getBoolean(Constants.USE_DICT, true);
