...
* 阅读只是一个转码工具，不提供内容，第一次安装app，需要自己手动导入站点，可以从公众号 **[开源阅读]**、QQ群、酷安评论里获取由书友制作分享的站点。
...
* 站点详情页规则添加刷新时重新获取目录页链接Url的配置,true or false
* 修复bug
...
* 源编辑添加cookieJar选项
* 源编辑菜单里添加清除cookie,如果之前能用的站点不能用了,可以关闭cookieJar后点下菜单里的清除cookie后再试
* 站点支持文件类型 by Xwite
* 启用firebase收集崩溃日志
...
* 引入compose包
* 站点类型为image的站点,翻译默认为滚动,图片样式默认为FULL
* url参数辅助输入
* 修复一些bug
...
* 站点编辑中的辅助键盘❓中可查看js说明文档 by Xwite
* 其它一些优化
