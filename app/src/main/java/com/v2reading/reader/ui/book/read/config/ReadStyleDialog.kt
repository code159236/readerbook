...
import com.v2reading.reader.databinding.DialogReadBookStyleBinding
import com.v2reading.reader.databinding.ItemReadStyleBinding
...
import com.v2reading.reader.ui.font.FontSelectDialog
...
class ReadStyleDialog : BaseDialogFragment(R.layout.dialog_read_book_style),
    ...
    private fun initViewEvent() = binding.run {
        ...
        fontSizeUp.setOnClickListener {
            ...
            postEvent(EventBus.UP_CONFIG, true)
            fontSize.text = ReadBookConfig.textSize.toString()
        }
        ...
        fabSearch.setOnClickListener {
            dismissAllowingStateLoss()
            postEvent(EventBus.GLOBAL_SEARCH, true)
        }

        fabAutoPage.setOnClickListener {
            dismissAllowingStateLoss()
            postEvent(EventBus.AUTO_PAGE, true)
        }

        setting.setOnClickListener {
            dismissAllowingStateLoss()
            postEvent(EventBus.OPEN_MORE_SETTING, true)
        }
    ...
    private fun upView() = binding.run {
        ...
        ReadBookConfig.let {
            fontSize.text = it.textSize.toString()
...
//                ivStyle.setTextColor(item.curTextColor())
                ...
                if (ReadBookConfig.styleSelect == holder.layoutPosition) {
                    ...
                    ivStyle.borderColor = 0x00000000
                }
            }
        }
