...
import com.v2reading.reader.ui.book.read.page.api.DataSource
import com.v2reading.reader.ui.book.read.page.delegate.*
...
import com.v2reading.reader.utils.activity
import com.v2reading.reader.utils.screenshot
...
class ReadView(context: Context, attrs: AttributeSet) :
    ...
    override fun onInterceptTouchEvent(event: MotionEvent?): Boolean {
        ...
        if (TextUtils.isEmpty(pageFactory.curPage.text)) {
            when (event?.action) {
                ...
                MotionEvent.ACTION_CANCEL, MotionEvent.ACTION_UP -> {
                    onTouchEvent(event)
                    return !TextUtils.isEmpty(pageFactory.curPage.text)
                }
            }
        }
        return true
            ...
            MotionEvent.ACTION_UP -> {
                ...
                if (!isMove) {
                    if (!longPressed && !pressOnTextSelected) {
                        return if (TextUtils.isEmpty(pageFactory.curPage.text)) {
                            false
                        } else {
                            onSingleTapUp()
                            true
