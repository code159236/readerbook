...
object ReadBook : CoroutineScope by MainScope() {
    ...
    fun upReadTime() {
        Coroutine.async {
            ...
            val format = AppConst.dateOnlyFormat.format(Date()) + PreferKey.todayReadTime
            val readTime = sp.getLong(format, 0)
            sp.edit().putLong(
                format,
    ...
    private fun download(
        ...
        if (book != null && bookSource != null) {
            ...
            contentLoadFinish(
                book, chapter, "没有站点", resetPageOffset = resetPageOffset
            ) {
                success?.invoke()
            }
            removeLoading(chapter.index)
