...
import com.v2reading.reader.data.entities.*
import com.v2reading.reader.help.coroutine.CompositeCoroutine
import com.v2reading.reader.model.rss.Rss
import com.v2reading.reader.model.webBook.WebBook
import com.v2reading.reader.utils.HtmlFormatter
    ...
    fun startDebug(scope: CoroutineScope, bookSource: BookSource, key: String) {
        ...
        when {
            ...
            key.contains("::") -> {
                ...
                log(bookSource.bookSourceUrl, "⇒开始访问站点页:$url")
        ...
        val info = WebBook.getBookInfo(scope, bookSource, book)
            .onSuccess {
                ...
                if (book.type != BookType.file) {
                    ...
                    log(debugSource, "≡文件类站点跳过解析目录", state = 1000)
                }
            }
            .onError {
                log(debugSource, it.msg, state = -1)
